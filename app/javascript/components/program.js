      ...
      <div className="program program-vertical">
        { this.props.program.terms.map((term) => (
          <div className="row" key={`row-${term}`}>
            <Term key={`orig-${term}`} courses={this.props.courses} editable={this.props.editable} term={this.props.program[term]} name={`Proposed ${term}`} />
            <Term key={`diff-${term}`} courses={this.props.courses} editable="false" term={this.props.diff[term]} name={`Current ${term}`} />
          </div>
        )) }
      </div>
    );
